@php
  ...
  $total = (int) ($total ?? 0);
  $status = isset($order) ? $order->status : 'pending';
  $createdAt = isset($order) ? $order->created_at : now('Asia/Jakarta');
  
  $statusIndex = array_search($status, ['pending', 'processing', 'shipped', 'completed']);
  if ($statusIndex === false) $statusIndex = 0;

  $trackingSteps = [
    ['l' => 'Order received',        's' => $statusIndex >= 0 ? ($statusIndex == 0 ? 'live' : 'done') : 'soon', 't' => $createdAt->format('d M · H:i')],
    ['l' => 'Atelier processing',    's' => $statusIndex >= 1 ? ($statusIndex == 1 ? 'live' : 'done') : 'soon', 't' => $createdAt->copy()->addDays(1)->format('d M')],
    ['l' => 'Dispatched from Yogyakarta','s' => $statusIndex >= 2 ? ($statusIndex == 2 ? 'live' : 'done') : 'soon','t' => $createdAt->copy()->addDays(3)->format('d M')],
    ['l' => 'Out for delivery / Delivered',      's' => $statusIndex >= 3 ? 'live' : 'soon', 't' => '~ ' . $createdAt->copy()->addDays(5)->format('d M')],
  ];
@endphp
          ...
          <div class="mt-6 relative">
            ...
            @foreach ($trackingSteps as $x)
              <div class="relative flex items-start gap-5 py-3">
                <span class="relative w-6 h-6 rounded-full grid place-items-center shrink-0 {{ in_array($x['s'], ['live','done']) ? 'bg-velora text-bone' : 'bg-ink/10' }}">
                  @if ($x['s'] === 'live')
                    <span class="absolute inset-0 rounded-full bg-velora pingdot"></span>
                    <svg class="relative" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="5" fill="currentColor"/></svg>
                  @elseif ($x['s'] === 'done')
